  ...
  rl.question("Enter your basic salary (in Ksh): ", function (basicSalary) {
    rl.question("Enter your benefits (in Ksh): ", function (benefits) {
      ...
      if (isNaN(basicSalary) || basicSalary < 0) {
        console.log(
          "Invalid input for basic salary. Please enter a valid number."
        );
        rl.close();
        return;
      }
...
function calculateTax(salary) {
  let tax = 0;
  if (salary <= 24000) {
    tax = salary * 0.1; // 10% tax for first 24,000
  } else if (salary <= 40000) {
    tax = 24000 * 0.1 + (salary - 24000) * 0.15; // 15% tax for next 16,000
  } else if (salary <= 80000) {
    tax = 24000 * 0.1 + 16000 * 0.15 + (salary - 40000) * 0.2; // 20% tax for next 40,000
  } else {
    tax = 24000 * 0.1 + 16000 * 0.15 + 40000 * 0.2 + (salary - 80000) * 0.3; // 30% tax for above 80,000
  }
  return tax;
}
